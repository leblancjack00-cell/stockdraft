...
function TickerRain() {
  const items = useRef(
    Array.from({ length: 24 }, (_, i) => ({
      ...
      pts: PTS[i % PTS.length],
      positive: PTS[i % PTS.length].startsWith('+'),
    ...
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none', zIndex: 0 }}>
      ...
      {items.map((item, i) => (
        <div key={i} style={{
          ...
          fontSize: item.size, color: item.positive ? '#00ff8818' : '#ff446618',
          fontFamily: font, fontWeight: 700, letterSpacing: '0.05em', whiteSpace: 'nowrap',
          animation: `fall ${item.duration}s ${item.delay}s infinite linear`,
        }}>
    ...
    <div style={{ fontFamily: font, background: '#080b14', color: '#c8d0e0', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 400, padding: 24 }}>
        ...
        <div style={{ background: '#0b1022', border: '1px solid #1a2040', borderRadius: 12, padding: 28, boxShadow: '0 24px 60px rgba(0,0,0,0.5)' }}>
          ...
          {/* Tabs */}
          <div style={{ display: 'flex', gap: 0, marginBottom: 24, borderBottom: '1px solid #1a2040' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              ...
              <div style={{ background: '#09091a', border: `1px solid ${emailFocus ? '#00bfff60' : '#1a2040'}`, borderRadius: 6, overflow: 'hidden', transition: 'border-color 0.15s' }}>
                <input
                  type="email" value={email} onChange={e => setEmail(e.target.value)}
                  onFocus={() => setEmailFocus(true)} onBlur={() => setEmailFocus(false)}
                  placeholder="you@example.com"
                  onKeyDown={e => e.key === 'Enter' && submit()}
                  style={{ width: '100%', padding: '13px 14px', background: 'transparent', border: 'none', outline: 'none', color: '#c8d0e0', fontSize: 13, fontFamily: font }}
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ background: '#09091a', border: `1px solid ${passFocus ? '#00bfff60' : '#1a2040'}`, borderRadius: 6, overflow: 'hidden', transition: 'border-color 0.15s', display: 'flex', alignItems: 'center' }}>
                <input
                  type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                  onFocus={() => setPassFocus(true)} onBlur={() => setPassFocus(false)}
                  placeholder="••••••••"
                  onKeyDown={e => e.key === 'Enter' && submit()}
                  style={{ flex: 1, padding: '13px 14px', background: 'transparent', border: 'none', outline: 'none', color: '#c8d0e0', fontSize: 13, fontFamily: font }}
                />
                <span onClick={() => setShowPass(s => !s)} style={{ padding: '0 14px', color: '#4a5568', cursor: 'pointer', fontSize: 10, letterSpacing: '0.08em', userSelect: 'none' }}>
                  {showPass ? 'HIDE' : 'SHOW'}
                </span>
              </div>
            </div>
            ...
            {error && (
              <div style={{ fontSize: 11, color: error.includes('Check your email') ? '#00ff88' : '#ff4466', padding: '8px 12px', background: error.includes('Check your email') ? '#0d1f15' : '#1a0d10', border: `1px solid ${error.includes('Check your email') ? '#00ff8830' : '#ff446630'}`, borderRadius: 4 }}>
                {error}
              </div>
            )}
          ...
          <div style={{ marginTop: 20, textAlign: 'center', fontSize: 10, color: '#2a3555' }}>
            ...
            <span onClick={() => { setMode(mode === 'signin' ? 'signup' : 'signin'); setError('') }} style={{ color: '#00bfff', cursor: 'pointer' }}>
        ...
        {/* Footer */}
        <div style={{ marginTop: 24, textAlign: 'center', fontSize: 10, color: '#2a3555', letterSpacing: '0.08em' }}>
          DRAFT STOCKS · SCORE DAILY · WIN YOUR LEAGUE
        </div>
      </div>
    </div>
  )
